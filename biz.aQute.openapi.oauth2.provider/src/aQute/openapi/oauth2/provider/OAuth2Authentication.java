...
@Component(service = {
		...
		OpenAPISecurityProvider.class
}, configurationPid = OAuth2Authentication.PID, configurationPolicy = ConfigurationPolicy.REQUIRE, property = "type=oauth2")
public class OAuth2Authentication implements OpenAPISecurityProvider {

	final static Logger			logger				= LoggerFactory.getLogger(OAuth2Authentication.class);
	...
	void activate(OAuth2Configuration config) throws Exception {
		...
		sessionKey = PID + "." + config.name();
		name = config.name();
		errorEndpoint = new URI(config.finalEndpoint());
		exposeHtmlResponse = config.exposeHtmlResponse();
	}
	...
	public URI login(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (user != null)
			return report(ErrorEnum.x_already_logged_in, user, response);
	...
	private URI report(ErrorEnum error, String error_description, HttpServletResponse response) throws URISyntaxException {
	...
	public void setEmail(String user, String email) {
		security.setProperty(user, handler.idKey, email);
	}
	...
	public OpenAPISecurityProviderInfo getInfo(HttpServletRequest request) {
		...
		info.name = name;
		info.type = "oauth2";
		info.idKey = handler.idKey;

		String user = (String) request.getSession().getAttribute(sessionKey);
		if ( user != null) {
			info.currentUser = user;
			info.idValue = security.getProperty(user, handler.idKey).orElse(null);
		}
