...
/**
 * <ul>
 * <li>{@link #uploadFile(long,Optional<String>,Optional<OpenAPIBase.Part>) POST
 * /pet/<b>[petId]</b>/uploadImage = uploadFile}
 * <li>{@link #findPetsByTags(List<String>) GET /pet/findByTags =
 * findPetsByTags}
 * <li>{@link #updatePet(Pet) PUT /pet = updatePet}
 * <li>{@link #getPetById(long) GET /pet/<b>[petId]</b> = getPetById}
 * <li>{@link #deletePet(Optional<String>,long) DELETE /pet/<b>[petId]</b> =
 * deletePet}
 * <li>{@link #addPet(Pet) POST /pet = addPet}
 * <li>{@link #findPetsByStatus(List<String>) GET /pet/findByStatus =
 * findPetsByStatus}
 * <li>{@link #updatePetWithForm(long,Optional<String>,Optional<String>) POST
 * /pet/<b>[petId]</b> = updatePetWithForm}
 * </ul>
 */
...
public abstract class GeneratedPet extends OpenAPIBase {
	...
	/**
	 * POST /pet/{petId}/uploadImage = uploadFile uploads an image
	 * 
	 ...
	 *            collectionFormat=%scsv
	 * @param file – file to upload (formData) collectionFormat=%scsv
	 * @returns 200 / successful operation 200
	 */

	protected abstract ApiResponse uploadFile(long petId, Optional<String> additionalMetadata,
			Optional<OpenAPIBase.Part> file) throws Exception;

	/**
	 * GET /pet/findByTags = findPetsByTags Finds Pets by tags Multiple tags can
	 * be provided with comma separated strings. Use tag1, tag2, tag3 for
	 * testing.
	 * 
	 * @param tags – Tags to filter by (query) collectionFormat=%scsv
	 * @returns 200 / successful operation
	 * @returns 400 / Invalid tag value 200 400
	 */

	protected abstract Iterable< ? extends Pet> findPetsByTags(List<String> tags)
			throws Exception, OpenAPIBase.BadRequestResponse;
	 ...
	 * @param body – Pet object that needs to be added to the store (body)
	 *            collectionFormat=%scsv
	 * @throws Response 405 / Invalid input 405
	 */

	protected abstract void addPet(Pet body) throws Exception;

	/**
	 * GET /pet/findByStatus = findPetsByStatus Finds Pets by status Multiple
	 * status values can be provided with comma separated strings
	 * 
	 * @param status – Status values that need to be considered for filter
	 *            (query) collectionFormat=%scsv
	 * @returns 200 / successful operation
	 * @returns 400 / Invalid status value 200 400
	 */

	protected abstract Iterable< ? extends Pet> findPetsByStatus(List<String> status)
			throws Exception, OpenAPIBase.BadRequestResponse;

	/**
	 * POST /pet/{petId} = updatePetWithForm Updates a pet in the store with
	 * form data
	 * 
	...
	protected abstract void updatePetWithForm(long petId, Optional<String> name, Optional<String> status)
			throws Exception;

	/**
	 * StatusEnum pet status in the store
	 */
	...
	public GeneratedPet() {
		super(BASE_PATH, aQute.openapi.example.petstore.GeneratedBase.class,
				...
				"updatePet            PUT    /pet  PAYLOAD Pet", "getPetById           GET    /pet/{petId}  RETURN Pet",
				"deletePet            DELETE /pet/{petId}", "addPet               POST   /pet  PAYLOAD Pet",
	...
	private void uploadFile_post_(OpenAPIContext context) throws Exception {
		...
		Optional<String> additionalMetadata_ = context
				.optional(context.toString(context.formData("additionalMetadata")));
		...
		if (context.require(status_, "status_")) {
			...
			for (String item_ : status_) {
				...
				if (context.require(item_, "item_")) {
					...
					context.validate(context.in(item_, "available", "pending", "sold"), item_, "item_",
							"context.in(item_, \"available\", \"pending\", \"sold\")");
				}
				context.end();
			}
		}
