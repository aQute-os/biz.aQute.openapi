 ...
 * <li>{@link #getOrderById(long) GET /store/order/<b>[orderId]</b> =
 * getOrderById}
 * <li>{@link #getInventory() GET /store/inventory = getInventory}
 * <li>{@link #deleteOrder(long) DELETE /store/order/<b>[orderId]</b> =
 * deleteOrder}
 * </ul>
 */
...
public abstract class GeneratedStore extends OpenAPIBase {
	 ...
	 * @returns 400 / Invalid Order 200 400
	 */

	protected abstract Order placeOrder(Order body) throws Exception, OpenAPIBase.BadRequestResponse;

	/**
	 * GET /store/order/{orderId} = getOrderById Find purchase order by ID For
	 * valid response try integer IDs with value <= 5 or > 10. Other values will
	 ...
	 * @returns 404 / Order not found 200 400 404
	 */
	 ...
	 * @throws Response 404 / Order not found 400 404
	 */
	...
	public static class Order extends OpenAPIBase.DTO {
		...
		public void validate(OpenAPIContext context, String name) {
			...
			if (this.quantity.isPresent()) {
				context.validate(this.quantity.get() <= 23, this.quantity.get(), "this.quantity.get()",
						"this.quantity.get() <= 23");
			}
			context.end();
		}
		...
		public Order shipDate(java.time.Instant shipDate) {
			this.shipDate = Optional.ofNullable(shipDate);
			return this;
		}

		public Optional<java.time.Instant> shipDate() {
