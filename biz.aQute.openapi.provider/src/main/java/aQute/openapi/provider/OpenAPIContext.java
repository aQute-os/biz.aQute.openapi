...
public class OpenAPIContext {
	...
	final static Logger				log				= LoggerFactory
			.getLogger(OpenAPIContext.class);
	private String					operation;
	private Method					method;
	private List<String>			stack			= new ArrayList<>();
	private List<String>			errors			= null;

	protected OpenAPIContext(OpenAPIRuntime runtime, HttpServletRequest request,
	...
	public void setResult(Object result, int resultCode) throws IOException {
		...
		if (result != null) {
			...
			try {
				OpenAPIRuntime.codec.enc().to(out).put(result);
			} catch (Exception e) {
	...
	public OpenAPIBase.Part part(String key)
	...
	public <T> T body(Class<T> type) throws Exception {
		...
		T t = OpenAPIRuntime.codec.dec().from(in).get(type);
		return t;
	}
	...
	public void validate(boolean expression, Object value, String reference,
			String validation) {
		if (expression)
			return;
	...
	private void securityException(String name, String message) {
