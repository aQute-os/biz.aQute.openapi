...
public class EnumTest extends gen.enums.EnumsBase {
	...
	public void testSimpleSerialization() throws Exception {
		AllEnumVariations aev = new aQute.json.codec.JSONCodec().dec().from("{\"simple\":\"a\"}")
				.get(AllEnumVariations.class);
	...
	public void testOptionalSimpleSerialization() throws Exception {
		AllEnumVariations aev = new aQute.json.codec.JSONCodec().dec().from("{\"optionalSimple\":\"h\"}")
				.get(AllEnumVariations.class);
	...
	public void testOptionalArraySerialization() throws Exception {
		AllEnumVariations aev = new JSONCodec().dec().from("{\"optionalArray\":[\"k\",\"l\"]}")
				.get(AllEnumVariations.class);
	...
	public void testWeirdMemberNamesEnum() throws Exception {
		AllEnumVariations aev = new JSONCodec().dec().from("{\"memberNames\":[\"new\",\"n^c\",\"m\",\"M\"]}")
				.get(AllEnumVariations.class);
		assertEquals(Arrays.asList(MemberNamesEnum.new$, MemberNamesEnum.n$5Ec, MemberNamesEnum.m, MemberNamesEnum.M),
				aev.memberNames);
	}
